...
public class ViewFeedbackActivity extends SherlockListActivity {
...
//		ActionBar ab = getActionBar();
		...
		initServiceBinder();
		initSendButton();
		EditText replyText = (EditText) findViewById(R.id.jconnect_droid_viewfeedback_reply_text);
...
//        getMenuInflater().inflate(R.menu.jconnect_feedback_menu, menu);
    	MenuInflater inflater = getSupportMenuInflater();
		inflater.inflate(R.menu.jconnect_feedback_menu, menu);
        return true;
    }
	...
	private void initIssue(Bundle state) {
		...
		if (issue == null) {
			throw new IllegalStateException(
					"Issue to display must be passed to this activity");
		}
	}
	...
	private Button findReplyButton() {
		return findView(this, R.id.jconnect_droid_viewfeedback_reply_button,
				Button.class);
	}

	private EditText findReplyTextView() {
		return findView(this, R.id.jconnect_droid_viewfeedback_reply_text,
				EditText.class);
	}
	...
	private class CommentsViewAdapter extends ArrayAdapter<Comment> {
		...
		public CommentsViewAdapter() {
			super(ViewFeedbackActivity.this,
					R.layout.jconnect_droid_view_feedback_list_item, 0,
					newArrayList(issue.getComments()));
		}

		@Override
		public View getView(int position, View convertView, ViewGroup parent) {
			View itemView = getOrInflate(getContext(),
					R.layout.jconnect_droid_view_feedback_list_item,
					convertView, parent);
			final Comment current = getItem(position);
			final String username = getUsername(current);
			final SpannableString contents = new SpannableString(username
					+ ": " + current.getText());
			contents.setSpan(new StyleSpan(Typeface.BOLD), 0,
					username.length() + 2, 0);
			findTextView(itemView, R.id.jconnect_droid_viewfeedback_contents)
					.setText(contents);
			findTextView(itemView, R.id.jconnect_droid_viewfeedback_date)
					.setText(formatDate(current));
			return itemView;
		}

		private String getUsername(Comment current) {
			return current.isSystemUser() ? getString(R.string.jconnect_droid_me)
					: current.getUsername();
		}
		...
		private CharSequence formatDate(Comment current) {
			...
			return getContext().getString(
					R.string.jconnect_droid_sent,
					DateFormat.getDateFormat(getContext()).format(
							current.getDate()));
		}
	}

	private static class ReplyCallback extends
			AbstractWrappingServiceCallback<ViewFeedbackActivity, Comment> {
